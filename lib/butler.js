...
dockerButler.prototype.runContainers = function() {
    ...
    return new Promise(function(resolve, reject) {
        ...
        if (self.config.env) {
            startOptions.Env = (typeof self.config.env === 'string' ? [self.config.env] : self.config.env);
        }

        self._docker.createContainer({Image: self.config.tag, Env: startOptions.Env}, function (err, container) {
            if(err) { return reject(err); }
